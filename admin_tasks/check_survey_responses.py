...
class DataCheckManager:
    ...
    def get_user_group_ids(self):
        self.user_group_ids = pg_utils.execute_query(
            sql_q.USER_GROUP_FOR_PROJECT_TASK, [self.project_task_id], jsonize_sql=False
        )
            ...
            for row in reader:
                ...
                if user_id is None:
                    ...
                    try:
                        ...
                        continue
                    user_id = pg_utils.execute_query(
                ...
                if (user_id in self.test_group_ids) and (
                    user_id in self.user_group_ids
                ):
                    self.logger.warning(
                    ...
                    if "@thisinstitute.cam.ac.uk" in user["email"]:
                        self.logger.error(f"User {user_id} is a member of THIS")
                    ...
                    err_message = (
                        f"User id {user_id} could not be found on user group or test group of project task {self.project_task_id}. Are "
    ...
    def main(self):
        ...
        (
            ...
            user_ids_in_input_file,
            this_users,
        ) = self.check_no_testers_in_data()
        self.logger.debug(
            "User ids in input file",
            extra={"user_ids_in_input_file": user_ids_in_input_file},
        )
        if not users_in_both_groups and not users_in_test_group and not this_users:
            print("Everything is OK!")
            print(
                "All users in Qualtrics survey responses file are members of the user group of this project task; "
                "none are members of the testing group; no responses from THIS Institute staff detected"
            )
        else:
            print("Problems found in input file!")
            if users_in_test_group:
                print(
                    "The following users are members of the test group of this project task:"
                )
                pprint(users_in_test_group)
            if users_in_both_groups:
                print(
                    "The following users are members of both the user and test groups of this project task:"
                )
                pprint(users_in_both_groups)
            if this_users:
                print("The following users are THIS Institute staff:")
                pprint(this_users)
            sys.exit(1)
...
if __name__ == "__main__":
    if SECRETS_NAMESPACE != "/prod/":
        continue_prompt = input(
            f"SECRETS_NAMESPACE is {SECRETS_NAMESPACE} instead of /prod/ so this will probably fail. "
