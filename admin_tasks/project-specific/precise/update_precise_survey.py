...
like_rephrasing_question_dict = {
    ...
    "Configuration": {
        ...
        "Autoscale": {"YScale": {"Name": "yesNo", "Type": "likert", "Reverse": False}},
    },
...
precise_indicator_phrasing_question_template = env.get_template(
    "precise_indicator_phrasing_question.j2"
)
...
class CsvParser:
    ...
    def parse_into_question_list(self):
        ...
        logger.debug(
            "Question list parsed from csv",
            extra={
                "question_list": [
                    [
                        x.data_export_tag,
                        x.original_phrasing,
                        x.rephrased_indicator,
                        x.explanatory_terms,
                        x.rationale_for_change,
                    ]
        ...
        for k, v in main_questions_for_blocks.items():
            ...
            block_elements_dict = {
                "BlockElements": [
                    ...
                    {
                        "Type": "Question",
                        "QuestionID": self.comment_box_questions[
                            f"{k}{COMMENT_QUESTION_POSTFIX}"
                        ]["QuestionID"],
                    },
                ]
            }
...
def main(
    ...
    rendered_questions_dict = {
        x.data_export_tag: x.rendered_question() for x in question_list
    }
    logger.debug("rendered_questions_dict", extra=rendered_questions_dict)
    update_manager = SurveyUpdateManager(
        rendered_questions_dict, survey_id=survey_id, survey_name=survey_name
    )
