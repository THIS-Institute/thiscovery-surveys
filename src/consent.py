...
class Consent:
    ...
    def as_dict(self):
        return {
            ...
            if (k[0] != "_") and (k not in ["created", "modified", "details", "type"])
        }
...
class ConsentEvent:
    def __init__(self, survey_consent_event):
        ...
        consent_dict[consent_embedded_data_fieldname] = json.loads(
            consent_dict[consent_embedded_data_fieldname]
        )
        try:
        ...
        try:
            consent_dict["consent_datetime"] = qualtrics2thiscovery_timestamp(
                consent_dict["consent_datetime"]
            )
        except KeyError:
        ...
        self.consent = Consent(
            core_api_client=self.core_api_client, correlation_id=self.correlation_id
        )
