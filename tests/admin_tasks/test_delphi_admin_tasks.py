...
from api.local.admin_tasks.qualtrics_admin.convert_responses_to_contact_list_input_format import (
    ResponsesToContactListConverter,
)
from api.local.admin_tasks.qualtrics_admin.republish_survey_with_user_specific_links import (
    DistributionLinksGenerator,
)
from api.local.admin_tasks.task_management.output_anon_project_specific_user_ids_for_test_group import (
    ProcessManager as OutputAnonIds,
)
from api.local.admin_tasks.task_management.create_user_group_for_follow_up_task import (
    ImportManager as CreateUserGroup,
)


class TestAdminTasksDbAccess(test_utils.DbTestCase):
    def test_generate_distribution_links_for_contact_list(self):
...
class TestAdminTasks(test_utils.BaseTestCase):
    def test_05_convert_qualtrics_responses_to_contact_list_import_format_ok(self):
        ...
        converter = ResponsesToContactListConverter(
            csvfile_path=os.path.join(
                test_utils.TEST_DATA_FOLDER, "qualtrics_responses.csv"
            )
        )
