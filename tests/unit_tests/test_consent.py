...
from src.common.constants import (
    CONSENT_ROWS_IN_TEMPLATE,
    DEFAULT_CONSENT_EMAIL_TEMPLATE,
)
...
class ConsentEventTestCase(test_utils.BaseTestCase):
    ...
    def test_01_init_ok_default_template(self):
        ...
        self.uuid_test_and_remove(
            entity_dict=consent_dict, uuid_attribute_name="consent_id"
        )
    ...
    def test_02_init_ok_custom_template(self):
        ...
        event["body"] = (
            f'{event["body"].rstrip("}")}, "template_name": "{custom_template}"' + "}"
        )
    ...
    def test_03_init_ok_missing_consent_datetime(self):
        ...
        event["body"] = event["body"].replace(
            '"consent_datetime":"2020-11-17T10:39:58+00:00",', ""
        )
        ce = ConsentEvent(event)
    ...
    def test_04_format_consent_statements_ok(self):
        ...
        self.assertDictEqual(
            expected_consent_rows_dict, self.ce._format_consent_statements()
        )
    ...
    def test_05_format_consent_statements_too_many_statements_raises_error(self):
        ...
        ce.consent.consent_statements = [
            {f"statement_{x}": "Yes"} for x in range(too_many_rows)
        ]
    ...
    def test_06_ddb_dump_and_load_ok(self):
        ...
        self.uuid_test_and_remove(
            entity_dict=loaded_consent_dict, uuid_attribute_name="project_task_id"
        )
        self.uuid_test_and_remove(
            entity_dict=loaded_consent_dict, uuid_attribute_name="project_id"
        )
