...
class TestUserInterviewTask(test_utils.BaseTestCase):
    def test_init_from_event_ok(self):
        uit = UserInterviewTask.from_eb_event(
            event=copy.deepcopy(td.TEST_USER_INTERVIEW_TASK_EB_EVENT)
        )
        self.assertEqual(
            td.TEST_USER_INTERVIEW_TASK_EB_EVENT["detail"]["response_id"],
            uit._response_id,
        )
...
class TestUserInterviewTaskEndpoint(test_utils.BaseTestCase, DdbMixin):
    ...
    def test_get_user_interview_task_ok(self):
        path_parameters = {"id": td.TEST_USER_INTERVIEW_TASK["response_id"]}
    ...
    def test_put_user_interview_task_ok(self):
        ...
        expected_item = {
            ...
            "details": {"event_type": "user_interview_task"},
