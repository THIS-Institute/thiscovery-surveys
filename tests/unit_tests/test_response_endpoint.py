...
class TestSurveyClient(BaseSurveyTestCase):
    ...
    def test_sc_01_get_response_ok(self):
        ...
        self.assertCountEqual(
            QUALTRICS_TEST_OBJECTS["unittest-survey-1"]["export_tags"],
            list(response.keys()),
        )

    def test_sc_02_get_response_ok_omit_null_values(self):
        response = self.survey_client.get_response(
            self.test_response_id, return_nulls=False
        )
        ...
        expected_keys = [
            x
            for x in QUALTRICS_TEST_OBJECTS["unittest-survey-1"]["export_tags"]
            if x not in questions_with_null_answers
        ]
...
class TestSurveyResponse(BaseSurveyTestCase):
    ...
    def test_sr_07_put_item_ok(self):
        ...
        self.assertEqual(
            HTTPStatus.OK, ddb_response["ResponseMetadata"]["HTTPStatusCode"]
        )
...
class TestResponseEndpoint(BaseSurveyTestCase):
    ...
    def test_ep_02_retrieve_response_api_ok_question_ids_not_specified(self):
        ...
        self.assertEqual(HTTPStatus.OK, result["statusCode"])
        expected_result_body_keys = QUALTRICS_TEST_OBJECTS["unittest-survey-1"][
            "export_tags"
        ]

        self.assertCountEqual(
            expected_result_body_keys, json.loads(result["body"]).keys()
        )
